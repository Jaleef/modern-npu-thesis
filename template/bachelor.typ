...
*第三点*：在存储层面`LEKV`选择了当前主流的基于 `LSM-Tree` @oneil1996lsm 的存储引擎 `LevelDB`，基于该引擎封装了需要的 `API` 如 *PUT / GET / DELETE / RangeQuery / RangeStats*，来给上层系统提供存储服务，`LevelDB`是一种内存存储结合硬盘持久化存储双重方式，数据先存入内存的跳表@pugh1990skip （Skip List）中，不会占用太多的系统 IO，内存的读写速度相比磁盘也更高速，当存储量超过一定阈值后，引擎将数据异步存入磁盘中持久化，不占用太多 CPU 时间，影响存储系统接收其他的请求。
...
第二章介绍了分布式关键的理论基础，以及`LEKV`用到分片算法和存储技术。

第三章介绍了`LEKV`对不同的模块选择的设计方案。

第四章介绍了`LEKV`在 `Linux` 操作系统环境下，使用 `C++` 语言实现上面的设计。
...
2000年，Eric Brewer 提出 CAP 猜想，由 Seth Gilbert 和 Nancy Lynch 给出形式化证明。CAP 定理指出：*一个分布式系统不可能同时满足一致性（Consistency）、可用性（Availability）和分区容错性（Partition Tolerance）这三个基本需求，最多只能同时满足其中两个*，该定理的核心概念有以下：

（1）*Consistency(一致性)*：所有节点在同一时刻看到的数据是一致的。即对某个 key 执行读操作，无论请求发送到哪个节点，返回的结果都是最近一次写入后的值

（2）*Availability(可用性)*：系统在任何时候都能响应客户端的请求，每个请求都能在优先时间内得到非错误的响应（不保证返回最新数据）。

（3）*Partition Tolerance(分区容错性)*：即使网络发生分区（部分节点之间无法通信），系统仍能继续运行。

CAP 定理的含义在于：*在网络可能发生分区的情况下，系统必须在一致性和可用性之间做出权衡*。由于网络分区在实际分布式环境中总是发生的，分布式系统设计者通常需要根据应用需求选择 `CP` 还是 `AP`。
...
*LEKV的设计选择*：它作为一个研究性质的系统，目前采用*AP偏向*的设计。Proxy 路由表更新通过 epoch 版本号控制，客户端缓存路由后直连 DataNode 操作，保证了AP偏向的高可用性。
...
BASE 理论强调高可用性和可扩展性，适用于对实时性一致性要求不那么高的场景，`LEKV`的客户端依照改理论，在首次访问时通过`Proxy`获取路由信息，之后将路由表缓存在本地，这意味着系统在`Tablet`分裂或迁移期间，客户端可能短暂将请求发送到错误的`DataNode`。系统通过 `ST_NOT_MY_SHARD`状态码通知客户端路由失效，客户端重新查询`Proxy`刷新路由表，最终在两次请求后达到最终一致性。

== 范围分片
数据分片（Data Sharding）是指将大规模数据集划分为多个较小的子集（或称为分片），分散存储在不同节点上，从而实现负载均衡和水平拓展。

=== 哈希分片（Hash-based Sharding）
哈希分片通过对 key 计算哈希值，再根据`DataNode`的个数取模来确定数据的存储位置。
 ```cpp
...
但是由于
范围查询时即使是相邻的数据也可能哈希到完全不同的节点，范围查询需要访问所有的节点。

（2）当节点增减时，哈希函数的输出会发生大规模变化，导致频繁的数据迁移。
...
*优点*是（1）范围查询相邻的`key`存储在同一节点上，范围查询只需要访问相关的数据节点。

（2）节点增加节点时只需迁移部分区间，不影响其他区间的数据。

（3）节点内的数据是有序的，可以高效地进行范围查询和统计。

（4）负载均衡以`Tablet`为单位迁移，无需迁移单个 key。

*缺点*是需要维护全局路由表，`Proxy`需要知道每个节点负责的`key`范围，路由表的更新会占用系统资源；需要额外的分裂机制，处理一个`Tablet`数据量过大时的情况；

`LEKV`的设计选择为*范围分片*策略，系统基于`LevelDB`存储引擎构建，其`LSM-Tree`结构天然保证 key 有序，范围分片可以充分利用这一特性实现高效的范围查询；范围分片是`Tablet`为单位，是独立可迁移的单元；即使需要维护全局路由表，也可以通过缓存机制减少路由查询，并且路由表的更新频率相对较低。 

== LevelDB 存储引擎
=== B+ 树 @bayer1972organization 
在分布式键值存储系统中，底层存储引擎的选择直接决定了系统的读写性能、数据持久化能力以及范围查询效率。传统关系型数据库普遍采用基于`B+`树的存储结构，`B+`树是一种自平衡的多路搜索树，是`B树` @bayer1972organization 的变体。其*核心特点*有：

- 所有数据存储在叶子节点，内部节点只存储键值和指向子节点的指针，不存储实际数据。
- 叶子节点形成有序链表，通过指针相连。
- 每个节点可以有多个子节点，树高度较低。
- 节点大小匹配磁盘页（如4KB）。
...
以上特点使得`B+`树能够高效地进行随机读写和范围查询，适合磁盘存储。但B+树在写入性能方面存在瓶颈，每次插入或删除操作都可能导致节点分裂或合并，引发大量的磁盘 I/O 操作，需要频繁地修改磁盘页，尤其在SSD 环境下会加速硬件磨损。

=== LSM-Tree
`LSM-Tree`是一种专门为写密集场景优化的数据结构，核心思想可以概括为"*延迟合并、顺序写入*"，与传统的`B+`树就地更新不同，它将所有对数据的写入操作转化为顺序追加操作。

`LSM-Tree`数据结构分两个层次，内存层次维护一个有序的数据结构`MemTable`，所有新写入的数据首先进入`MemTable`，当它达到预设大小阈值后，被标记为`Immutable MemTable`（只读），同时新的 `MemTable`被创建以继续接收写入。后台线程负责将`Immutable MemTable`顺序刷写到磁盘，生成不可变的`SSTable`（Sorted String Table）文件。磁盘上的`SSTable`按照生成时间组织成多个层级（Level-0 到 Level-N），层级越深的 SSTable 文件越大、数据越旧。
...
读路径则是一个从上至下的搜索过程：首先查找活跃的 MemTable，其次查找 Immutable MemTable（如果存在），然后按时间从新到旧依次查找各级 SSTable。为了加速读操作，LevelDB 为每个 SSTable 文件维护了布隆过滤器 @bloom1970space（Bloom Filter），可以快速判断某个键是否可能存在于该文件中。
...
因此，路由集中由 Proxy 集中管理使得分片调度策略的实现更为简单，所有元数据变更（如 Tablet 分裂、节点增减）都由 Proxy 统一处理，DataNode 只需关注数据存储和查询逻辑；客户端仅需要知晓 Proxy 地址即可接入系统，DataNode 的动态变化对客户端透明。

LEKV 由一下两种模块构成：

*Proxy 节点（端口9001）*：
- 维护全局 Tablet 路由表，记录每个 Tablet 的区间范围与负责节点映射
- 响应客户端的路由查询请求（OP_GET_ROUTE），返回目标 DataNode 的地址信息
- 响应客户端的全量路由表请求（OP_SHARDS），返回当前所有 Tablet 的路由信息，供客户端缓存使用
- 运行 BalancerLoop 后台线程，周期性执行 Tablet 统计、自动分裂与负载均衡。
- 维护到所有 DataNode的持久 TCP 连接，进行分片调度和数据迁移。

*DataNode 节点（端口自动从9002开始拓展）*：
- 运行 LevelDB 存储引擎，提供键值读写与范围查询功能。
- 响应客户端的数据操作请求（OP_PUT / OP_GET / OP_DELETE / OP_RANGE_QUERY / OP_RANGE_STATS），执行对应的存储引擎操作并返回结果。
- 响应 Proxy 的内部数据统计请求（OP_TABLET_STATS），返回区间内键值数量和中位数键。
- 响应 Proxy 的数据迁移请求 （OP_SCAN_RANGE），返回指定范围内的所有键值对。

*客户端（lekv_cli）*：
- 启动时通过 Proxy 获取全部路由信息，缓存 Tablet 路由表到本地。
- 对每个数据操作请求，根据缓存的路由表计算目标 DataNode 地址，直接与 DataNode 通信执行操作。
- 在收到 ST_NOT_MY_SHARD 错误码响应时，重新查询 Proxy 刷新路由表后重试请求。
...
其中空字符串 "" 代表 key 的字典序最小值，"m" 代表两个区间的分界点。选择字母表均分在于，英文字母在键值存储系统的常见键命名种有较好的分布特性，且"m" 是一个相对均衡的分割点；每个 Tablet 都有一个唯一的 TabletID 和一个 epoch 版本号，Proxy 维护一个全局路由表记录每个 Tablet 的区间范围、负责节点和版本信息。后续的自动分裂机制会根据实际数据分布动态调整边界。
...
二分查找算法的优势在于其高效性和稳定性，能够快速定位到正确的 Tablet，尤其在 Tablet 数量较多时表现更为明显。相比于线性查找，二分查找大大减少了平均查找次数。
...
Proxy 会从负载最高的节点上选择一个`key_count`最大的 Tablet 进行迁移，迁移过程分为两步 *Scan + Transfer*：
...
+ *Transfer 阶段*：Proxy 收到响应后，解析出所有键值对，然后向目标节点发送一系列 `OP_PUT` 请求，将这些键值对写入目标节点的 LevelDB 中。每个 `OP_PUT` 请求包含一个键值对的完整信息，DataNode 接收到请求后执行写入操作并返回结果。每条 PUT 都等待目标节点的 `ST_OK` 确认后才发送下一条。
...
== 本章小结
本章介绍了 LEKV 系统的设计细节，第一，系统采用了 Proxy + DataNode 的双层架构，将路由管理与数据存储分离；第二，设计高效的自定义二进制通信协议，解决了 TCP 粘包问题；第三，在分片与路由机制方面，采用基于字母表范围均分的初始分片策略，并通过二分查找实现高效的路由定位，同时设计了客户端缓存路由表和失效刷新机制；第四，系统实现了基于键数量差值比较的负载均衡策略，通过 Scan + Transfer 机制进行数据迁移，并确保路由表的原子更新。
